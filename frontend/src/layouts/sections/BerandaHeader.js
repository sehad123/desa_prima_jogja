...
import { faBars, faUser, faCaretDown, faCaretUp, faTimes, faSignOutAlt } from "@fortawesome/free-solid-svg-icons";
      ...
      <header className="bg-primary top-0 left-0 font-poppins fixed w-full z-50 bg-opacity-95 border-b border-gray-200 shadow-sm">
        <div className="flex h-16 items-center pl-2 pr-2 md:pr-3">
          ...
          <div className="md:flex items-center space-x-4 lg:mr-10 hidden relative">
            <div className="flex items-center cursor-pointer" onClick={handleProfileDropdownToggle}>
              <FontAwesomeIcon icon={faUser} className="text-black text-2xl border border-gray-400 p-2 rounded-full" />
            ...
            {isProfileDropdownOpen && (
              <div className="absolute right-0 top-11 mt-2 w-40 pl-3 bg-white border rounded-lg">
                ...
                <a onClick={handleLogout} className="block px-4 py-2 -ml-4 text-sm text-black hover:bg-gray-200 cursor-pointer">
        ...
        <aside className="fixed lg:mt-12 w-full md:w-80 bg-white h-full shadow-xl z-40 transform transition-transform duration-300 ease-in-out overflow-x-scroll">
          <nav className="mt-20 md:mt-3 md:ml-0">
            <div className="flex flex-col items-center md:hidden pb-2 border-b border-grey-200">
              <div className="flex space-x-6 cursor-pointer " onClick={handleMobileProfileDropdownToggle}>
                ...
                <FontAwesomeIcon icon={isMobileProfileDropdownOpen ? faCaretUp : faCaretDown} className="text-black text-xl ml-2" />
              </div>
              {isMobileProfileDropdownOpen && (
                <div className="w-full bg-white py-2 text-center cursor-pointer">
                  <a href="/profil" className="block px-4 py-2 text-black hover:bg-gray-200">
                    <FontAwesomeIcon icon={faUser} className="mr-2" />
                    Profil
                  </a>
                  <a onClick={handleLogout} className="block px-4 py-2 text-black hover:bg-gray-200 cursor-pointer">
                    <FontAwesomeIcon icon={faSignOutAlt} className="mr-2 cursor-pointer" />
                    Keluar
                  </a>
                </div>
              )}
