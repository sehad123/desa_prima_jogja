...
const KabupatenDetail = () => {
  ...
  const countDesaPerTimeline = (timeline, kategori) => {
    return timeline.map((date) => {
      return (kabupaten.desaList ?? []).filter((desa) => {
        ...
        return desa.kategori.toLowerCase() === kategori.toLowerCase() && desaStart <= timelineDate;
      }).length;
    });
  };
  ...
  //         (date) =>
  //           countDesaPerTimeline(timelineLabels, "Maju")[timelineLabels.indexOf(date)] +
  //           countDesaPerTimeline(timelineLabels, "Berkembang")[timelineLabels.indexOf(date)] +
  //           countDesaPerTimeline(timelineLabels, "Tumbuh")[timelineLabels.indexOf(date)]
  //       ),
  //       borderColor: "#3F51B5",
  ...
  const lineChartData = {
    ...
    datasets: [
      {
        label: "Maju",
        data: timelineLabels.map(() => kabupaten.jumlah_maju), // Gunakan jumlah desa dari API
        borderColor: "#4CAF50",
        backgroundColor: "rgba(76, 175, 80, 0.2)",
        fill: false,
      },
        ...
        <div className="bg-white p-4 rounded-md">
          <div className="px-2 flex justify-between items-center mb-4">
            ...
            <Link to={`/kelompok-desa?kabupaten=${kabupaten.nama_kabupaten === "Kota Yogyakarta" ? kabupaten.nama_kabupaten : `KAB. ${kabupaten.nama_kabupaten.toUpperCase()}`}`}>
              <button className="bg-blue-700 text-white text-xs lg:text-lg py-2 px-4 rounded-md shadow-md">Daftar Kelompok</button>
            </Link>
          </div>
