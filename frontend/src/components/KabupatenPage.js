...
const KabupatenPage = () => {
  ...
  const RealColor = ({ numReal, numTarget }) => {
    ...
    if (num < 0.25) {
      return (
        <>
          <span className="text-red-900 bg-red-100 px-2 ml-1 rounded-md "> {numReal} </span>
          <span className="ml-auto text-xl font-bold text-red-600">{(num * 100).toFixed(0)}%</span>
        </>
      );
    } else if (num < 0.5) {
      return (
        <>
          <span className="text-orange-900 bg-orange-200 px-2 ml-1 rounded-md "> {numReal} </span>
          <span className="ml-auto text-xl font-bold text-orange-400">{(num * 100).toFixed(0)}%</span>
        </>
      );
    } else if (num < 0.75) {
      return (
        <>
          <span className="text-yellow-900 bg-yellow-100 px-2 ml-1 rounded-md "> {numReal} </span>
          <span className="ml-auto text-xl font-bold text-yellow-300">{(num * 100).toFixed(0)}%</span>
        </>
      );
    } else if (num < 1) {
      return (
        <>
          <span className="text-blue-900 bg-blue-100 px-2 ml-1 rounded-md "> {numReal} </span>
          <span className="ml-auto text-xl font-bold text-blue-600">{(num * 100).toFixed(0)}%</span>
        </>
      );
    } else {
      return (
        <>
          <span className="text-green-900 bg-green-100 px-2 ml-1 rounded-md "> {numReal} </span>
          <span className="ml-auto text-xl font-bold text-green-600">{(num * 100).toFixed(0)}%</span>
        </>
      );
    }
  };
      ...
      <div className="p-5">
        ...
        <div className="mt-6">
          <div className="flex justify-between items-center p-4 bg-white">
            <h1 className="text-lg lg:text-xl font-medium text-gray-800">Daftar Kabupaten/Kota Provinsi D.I. Yogyakarta</h1>
            <button onClick={() => navigate(`/peta-desa`)} className="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 flex items-center text-sm lg:text-lg">
              Dashboard
            </button>
          </div>
        </div>
        ...
        <div className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 cursor-pointer bg-white">
          {kabupatenList.map((kabupaten) => {
            ...
            return (
              <div key={kabupaten.id} className="bg-white rounded-lg shadow-lg p-4 border-[1px] border-gray-300 relative" onClick={() => navigate(`/detail/${kabupaten.id}`)}>
                ...
                <div className="mb-2 text-slate-800 text-sm">
                  ...
                  <div className="flex items-center">
                    ...
                    <p className="flex">:</p> <RealColor numReal={totalKelompok} numTarget={kabupaten.jumlah_desa} />
                  </div>
                </div>
              </div>
            );
