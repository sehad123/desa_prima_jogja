...
const Table = ({ columns = [], initialData = [], isMobile, onUpdate }) => {
  ...
  const confirmStatusChange = async () => {
    try {
      setUpdating(true);
      const response = await axios.patch(`http://localhost:5000/api/desa/${selectedRow.id}/status`, { status: newStatus });

      if (onUpdate) {
        await onUpdate();
      }

      toast.success(`Status ${selectedRow.nama} berhasil diubah menjadi ${newStatus}`);
      closeStatusModal();
    } catch (error) {
    ...
    return (
      <>
        ...
        {pageNumbers.map((pageNumber) => (
          <button key={pageNumber} onClick={() => handleChangePage(pageNumber)} className={`py-1 px-3 border rounded-md ${page === pageNumber ? "bg-secondary text-white" : "bg-gray-100 hover:bg-gray-300"}`}>
            {pageNumber + 1}
          </button>
        ))}
  ...
  const renderStatusBadge = (status) => {
    switch (status) {
      case "Disetujui":
        return <span className="bg-green-100 text-green-800 text-xs font-medium px-2.5 py-0.5 rounded-full">Disetujui</span>;
      case "Ditolak":
      ...
      <div className="w-full px-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
          {sortedData.length > 0 ? (
              ...
              <div key={index} className="bg-white rounded-xl shadow-md overflow-hidden border border-gray-200 hover:shadow-lg transition-shadow duration-300">
                <div className="p-5">
                  <div className="flex justify-between items-start mb-3">
                    <h3 className="text-lg font-bold text-gray-800">{truncateText(row.nama, 30)}</h3>
                    <div>
                      {profil.role === "Pegawai" ? (
                        <div className="flex justify-center space-x-2">
                          <button className={`py-1 px-2 rounded-md ${row.status === "Disetujui" ? "bg-green-500 text-white" : "bg-gray-300"}`} onClick={() => openStatusModal(row, "Disetujui")}>
                            <FontAwesomeIcon icon={faCheck} />
                          </button>
                          <button className={`py-1 px-2 rounded-md ${row.status === "Ditolak" ? "bg-red-500 text-white" : "bg-gray-300"}`} onClick={() => openStatusModal(row, "Ditolak")}>
                        ...
                        <div className="flex justify-center space-x-2">
                          {row.status === "Disetujui" && <FontAwesomeIcon icon={faCheck} className="text-green-500" />}
                          {row.status === "Ditolak" && <FontAwesomeIcon icon={faBan} className="text-red-500" />}
                          {row.status === "" || row.status === "Pending" ? <FontAwesomeIcon icon={faClock} className="text-gray-500" /> : null}
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 text-sm text-gray-600">
                    ...
                    <div className="flex items-center">
                      <FontAwesomeIcon icon={faPeopleGroup} className="mr-2 text-gray-400" />
                  ...
                  <div className="mt-4 pt-3 border-t border-gray-200">
                    <div className="flex items-center text-sm">
                      <FontAwesomeIcon icon={faPenToSquare} className="mr-2 text-gray-400" />
                      {profil.role === "Pegawai" ? (
                        <button onClick={() => openNoteModal(row)} className="text-purple-600 hover:text-purple-800 text-sm">
                          {row.catatan ? "Edit Catatan" : "Tambah Catatan"}
                        </button>
                      ) : (
                        <span className="text-gray-600">{truncateText(row.catatan, 60) || "Tidak ada catatan"}</span>
                      )}
                    </div>
                  </div>
            ...
            <div className="col-span-full flex flex-col items-center justify-center h-64 bg-white rounded-lg shadow-sm text-gray-500">
              <FontAwesomeIcon icon={faMagnifyingGlass} className="text-gray-400 mb-3" size="2x" />
              <p className="text-center max-w-md">Data tidak ditemukan. Silakan coba dengan filter atau kata pencarian yang berbeda.</p>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center p-4 bg-white rounded-lg shadow-sm">
          ...
          <div className="flex items-center space-x-2">
            <select value={rowsPerPage} onChange={handleChangeRowsPerPage} className="py-1 px-3 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-secondary">
              {[10, 25, 50].map((option) => (
  ...
  return (
    <div>
        ...
        <table className="min-w-full bg-white border-collapse text-sm">
              ...
              {columns.map((column) => (
                ...
                >
                  <div className="flex items-center justify-start">
                    {column.id !== "actions" && (
                      <FontAwesomeIcon
                        icon={sortConfig.key === column.id ? (sortConfig.direction === "ascending" ? faArrowUp : faArrowDown) : faArrowUp}
                        className={`absolute left-4 ${sortConfig.key === column.id ? "text-black" : "text-gray-400"}`}
                      />
                    )}
                    <div className="ml-4">
                      <span className={`ml-${column.id !== "actions" ? "6" : "0"}`}>{column.label}</span>
                    </div>
                  </div>
                </th>
              ))}
          ...
          <tbody>
            {sortedData.length > 0 ? (
              sortedData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, index) => (
                <tr key={index} className="hover:bg-gray-100 rounded-full">
                  {columns.map((column) => {
                    ...
                    const alignment = column.id === "nama" || column.id === "alamat" ? "text-left" : "text-center";

                    return (
                      <td key={column.id} className={`py-2 px-4 border-b-8 font-light border-base whitespace-nowrap ${alignment}`}>
                  ...
                  <td className="py-2 px-4 text-center border-b-8 font-light border-base whitespace-nowrap">
                    ...
                    {profil.role === "Pegawai" ? (
                      ...
                      <div className="py-2 px-4 text-center font-light">{row.catatan}</div>
                    )}
                  </td>
                  ...
                  <td className="py-2 px-4 text-center border-b-8 font-light border-base sticky right-0 bg-white" style={{ zIndex: 1 }}>
                    <Link to={`/kelompok-desa/${row.id}`}>
                      <button className="bg-purple-700 text-white py-1 px-2 rounded-md">Detail</button>
                    </Link>
                  </td>
                </tr>
              ))
              ...
              <tr>
                <td colSpan={columns.length + 3} className="text-center py-5 px-4">
      ...
      {showStatusModal && selectedRow && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg max-w-md w-full">
            ...
            <p>
              Apakah Anda yakin ingin mengubah status kelompok desa <strong>{selectedRow.kelompok_desa}</strong> menjadi <strong>{newStatus}</strong>?
            </p>
            <div className="flex justify-end space-x-3 mt-4">
              <button onClick={closeStatusModal} className="px-4 py-2 bg-gray-300 rounded-md">
                Batal
              </button>
              <button onClick={confirmStatusChange} disabled={updating} className={`px-4 py-2 rounded-md text-white ${newStatus === "Disetujui" ? "bg-green-500" : "bg-red-500"}`}>
      ...
      {showNoteModal && selectedRow && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg max-w-md w-full">
            <h3 className="text-lg font-semibold mb-4">Tambahkan Catatan untuk {selectedRow.kelompok_desa}</h3>
            <textarea value={noteInput} onChange={(e) => setNoteInput(e.target.value)} className="w-full p-2 border border-gray-300 rounded-md h-32" placeholder="Masukkan catatan..." />
            <div className="flex justify-end space-x-3 mt-4">
              <button onClick={closeNoteModal} className="px-4 py-2 bg-gray-300 rounded-md">
                Batal
              </button>
              <button onClick={submitNote} disabled={updating} className="px-4 py-2 bg-secondary text-white rounded-md">
